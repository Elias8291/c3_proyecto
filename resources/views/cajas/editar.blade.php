...
@section('css')
<!-- Reutilizamos el CSS de la página de creación -->
<style>
    .side-menu {
        padding: 0;
        margin: 0;
    ...
    <section class="page-background" style="background: transparent">
        <div class="container">
            ...
            <form action="{{ route('cajas.update', $caja->id) }}" method="POST" id="cajaForm">
                @csrf
                @method('PATCH') <!-- Usamos el método PATCH para la actualización -->
                ...
                <div class="form-row">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input name="numero_caja" id="numero_caja" value="{{ old('numero_caja', $caja->numero_caja) }}"
                            class="form-control @error('numero_caja') form-error @enderror" type="number" min="1" required>
                ...
                <div class="form-group mb-4">
                    ...
                    <select name="ubicacion" class="form-control @error('ubicacion') form-error @enderror" required>
                        ...
                        <option value="Armario {{ $num }} - Lado A" {{ old('ubicacion', $caja->ubicacion)=="Armario $num - Lado A" ? 'selected' : '' }}>Armario {{ $num }} - Lado A</option>
                        <option value="Armario {{ $num }} - Lado B" {{ old('ubicacion', $caja->ubicacion)=="Armario $num - Lado B" ? 'selected' : '' }}>Armario {{ $num }} - Lado B</option>
                        @endforeach
                    </select>
                    @error('ubicacion')
                    <p class="form-error">{{ $message }}</p>
                ...
                <div class="form-group mb-4">
                    ...
                    <input name="maximo_carpetas" value="{{ old('maximo_carpetas', $caja->maximo_carpetas) }}"
                        class="form-control @error('maximo_carpetas') form-error @enderror" type="number" min="1" required>
                ...
                <div class="form-preview">
                    <div class="preview-content">
                        <div class="preview-item" data-type="numero">
                            ...
                            <span class="preview-value" id="previewNumeroCaja">{{ $caja->numero_caja }}</span>
                        </div>
                        
                        <div class="preview-item" data-type="mes">
                            <div class="preview-icon"></div>
                            <span class="preview-label">Mes</span>
                            <span class="preview-value" id="previewMes">{{ $caja->mes }}</span>
                        </div>
                        
                        <div class="preview-item" data-type="anio">
                            <div class="preview-icon"></div>
                            <span class="preview-label">Año</span>
                            <span class="preview-value" id="previewAnio">{{ $caja->anio }}</span>
                        </div>
                        
                        <div class="preview-item" data-type="ubicacion">
                            <div class="preview-icon"></div>
                            <span class="preview-label">Ubicación</span>
                            <span class="preview-value" id="previewUbicacion">{{ $caja->ubicacion }}</span>
                        </div>
                        
                        <div class="preview-item" data-type="carpetas">
                            <div class="preview-icon"></div>
                            <span class="preview-label">Máx. Carpetas</span>
                            <span class="preview-value" id="previewMaximoCarpetas">{{ $caja->maximo_carpetas }}</span>
                        </div>
                    </div>
                </div>
                <button type="submit" class="btn-submit">Actualizar Caja</button>
            </form>
        </div>
    </section>
</main>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const maximoCarpetasInput = document.querySelector('input[name="maximo_carpetas"]');
