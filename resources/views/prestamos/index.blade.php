            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="actions-container">
                        <div class="d-flex align-items-center">
                            <a class="btn btn-new" href="{{ route('prestamos.create') }}">
                        ...
                        <div class="table-responsive">
                            <table class="table" id="miTabla2">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $prestamo->id }}</td>
                                        <td>{{ $prestamo->documento->numero_hojas }} hojas</td>
                                        <td>{{ $prestamo->usuario ? $prestamo->usuario->name : 'Usuario no encontrado' }}</td>

                                        <td>{{ $prestamo->fecha_solicitud->format('d/m/Y') }}</td>
                                        <td>
                                            <span class="badge badge-{{ $prestamo->estado == 'Pendiente' ? 'warning' : ($prestamo->estado == 'Aprobado' ? 'success' : 'danger') }}">
                                                {{ $prestamo->estado }}
                                            </span>
                                        </td>
                                        <td>
                                            <div class="action-buttons">
                                                @if($prestamo->estado == 'Pendiente')
                                                    <form action="{{ route('prestamos.aprobar', $prestamo) }}" method="POST" style="display:inline;">
                                                        @csrf
                                                        <button type="submit" class="btn btn-success">
                                                            <i class="fas fa-check"></i> Aprobar
                                                        </button>
                                                    </form>
                                                    <form action="{{ route('prestamos.rechazar', $prestamo) }}" method="POST" style="display:inline;">
                                                        @csrf
